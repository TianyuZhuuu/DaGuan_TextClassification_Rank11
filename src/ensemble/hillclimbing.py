...
if __name__ == '__main__':
    ...
    for num_leaves in [7, 31, 127]:
        for seed in range(1, 11):
            for name in os.listdir(f'../../oof_pred/layer1_lgbm_{num_leaves}leaves/{seed}'):
                if 'train' in name:
                    train_preds.append(np.load(f'../../oof_pred/layer1_lgbm_{num_leaves}leaves/{seed}/{name}'))
                elif 'test' in name:
                    test_preds.append(np.load(f'../../oof_pred/layer1_lgbm_{num_leaves}leaves/{seed}/{name}'))

    faster_hill_climbing_ensemble(model_names, train_preds, test_preds, n_iter=100, file_name='reproduce_all_depth_merged_lgbm')
